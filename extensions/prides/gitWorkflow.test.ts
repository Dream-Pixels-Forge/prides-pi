...
describe("gitWorkflow — pure domain logic", () => {
	...
	it("checkBranchConforms validates PRIDES taxonomy", () => {
		// Main branches always conform
		expect(checkBranchConforms("main").ok).toBe(true);
		expect(checkBranchConforms("master").ok).toBe(true);
		expect(checkBranchConforms("develop").ok).toBe(true);

		// Prefixed branches conform
		expect(checkBranchConforms("feature/add-login").ok).toBe(true);
		expect(checkBranchConforms("hotfix/auth-leak").ok).toBe(true);
		expect(checkBranchConforms("bug/null-pointer").ok).toBe(true);
		expect(checkBranchConforms("release/v2.0").ok).toBe(true);
		expect(checkBranchConforms("chore/update-deps").ok).toBe(true);
		expect(checkBranchConforms("docs/readme").ok).toBe(true);

		// Non-prefixed non-main branches do NOT conform
		expect(checkBranchConforms("my-feature").ok).toBe(false);
		expect(checkBranchConforms("random-branch").ok).toBe(false);
		expect(checkBranchConforms("auth-module").ok).toBe(false);
	});
});
