...
export function checkBranchConforms(
	...
	// Non-main branches must use a prefix (feature/, hotfix/, bug/, release/, chore/)
	if (!name.includes("/")) {
		return {
			ok: false,
			type,
			reason: `Branch '${name}' does not follow PRIDES taxonomy. Use a prefix: feature/, hotfix/, bug/, release/, or chore/`,
		};
	}
