...
export function canAdvance(state: PRIDESState, defs: GateDef[]): AdvanceCheck {
	...
	// I→D 100% task completion gate: all tasks created during Implement phase
	// must be marked completed before advancing to Deploy.
	if (state.phase === "I" && next === "D") {
		const implTasks = state.tasks.filter((t) => t.phase === "I");
		if (implTasks.length > 0) {
			const incomplete = implTasks.filter((t) => t.status !== "completed");
