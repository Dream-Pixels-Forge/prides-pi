...
describe("PRIDESEngine", () => {
	...
	it("autoDetectGit returns not-detected when no git state exists", () => {
		const { engine } = makeEngine();
		const result = engine.autoDetectGit();
		expect(result.detected).toBe(false);
		expect(result.branchName).toBeNull();
	});

	it("autoDetectGit returns tracked state when git already initialized", () => {
		const { engine } = makeEngine();
		engine.startGitBranch("feature/auth");
		const result = engine.autoDetectGit();
	...
	it("initGitFromBranch initializes git and validates taxonomy", () => {
		const { engine } = makeEngine();

		// Conformant branch
		const good = engine.initGitFromBranch("feature/add-login");
		expect(good.ok).toBe(true);
		expect(good.conforms).toBe(true);
		expect(engine.state.git?.currentBranch).toBe("feature/add-login");
		expect(engine.state.git?.branchType).toBe("feature");
		expect(engine.state.git?.step).toBe("branch");

		// Non-conformant branch (still tracked, but warns)
		const bad = engine.initGitFromBranch("random-branch");
		expect(bad.ok).toBe(true);
		expect(bad.conforms).toBe(false);
		expect(bad.message).toContain("does not follow PRIDES taxonomy");
		expect(engine.state.git?.currentBranch).toBe("random-branch");
		expect(engine.state.git?.branchType).toBe("feature");
	});

	it("initGitFromBranch handles main branches", () => {
		const { engine } = makeEngine();
		const result = engine.initGitFromBranch("main");
		expect(result.ok).toBe(true);
		expect(result.conforms).toBe(true);
		expect(engine.state.git?.branchType).toBe("main");
	});

	// --- Warning tests ---

	it("addWarning creates a warning and resolveWarning resolves it", () => {
		const { engine } = makeEngine();
		const w = engine.addWarning("error", "gate-failure", "test failed");
		expect(w.severity).toBe("error");
		expect(w.category).toBe("gate-failure");
		expect(w.message).toBe("test failed");
		expect(engine.getActiveWarnings()).toHaveLength(1);

		const r = engine.resolveWarning(w.id);
		expect(r.ok).toBe(true);
		expect(engine.getActiveWarnings()).toHaveLength(0);
	});

	it("getWarningsBySeverity filters correctly", () => {
		const { engine } = makeEngine();
		engine.addWarning("error", "a", "err1");
		engine.addWarning("warn", "b", "warn1");
		engine.addWarning("info", "c", "info1");

		expect(engine.getWarningsBySeverity("error")).toHaveLength(1);
		expect(engine.getWarningsBySeverity("warn")).toHaveLength(1);
		expect(engine.getWarningsBySeverity("info")).toHaveLength(1);
	});

	it("hasBlockingWarnings returns true for warn/error", () => {
		const { engine } = makeEngine();
		expect(engine.hasBlockingWarnings()).toBe(false);

		engine.addWarning("info", "a", "info only");
		expect(engine.hasBlockingWarnings()).toBe(false);

		engine.addWarning("warn", "b", "a warning");
		expect(engine.hasBlockingWarnings()).toBe(true);
	});

	it("shouldBlockGitOps blocks on failing gates", async () => {
		const { engine } = makeEngine({ code: 1 });
		engine.setPhase("I");
		await engine.runGate("linter");
		...
		expect(check.reason).toContain("linter");
	});

	it("shouldBlockGitOps blocks on blocking warnings", () => {
		const { engine } = makeEngine();
		engine.addWarning("error", "security", "vulnerability found");

		const check = engine.shouldBlockGitOps();
		expect(check.blocked).toBe(true);
		expect(check.reason).toContain("security");
	});

	it("shouldBlockGitOps blocks on non-conformant branch", () => {
		const { engine } = makeEngine();
		engine.initGitFromBranch("random-branch");

		const check = engine.shouldBlockGitOps();
		expect(check.blocked).toBe(true);
		expect(check.reason).toContain("taxonomy");
	});

	it("shouldBlockGitOps allows when all clear", () => {
		const { engine } = makeEngine();
		engine.startGitBranch("feature/auth");
